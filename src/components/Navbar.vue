<template>
  <nav :class="['navbar', { scrolled: isScrolled }]">
    <div class="navbar-inner">
      ...
      <div class="nav-links">
        <RouterLink to="/calendario" class="nav-link">
      ...
      <RouterLink to="/calendario" class="btn-nav">
        Ver meu calendário
      </RouterLink>
    ...
    <Transition name="slide-up">
      <div v-if="menuOpen" class="mobile-menu">
        <RouterLink to="/calendario" class="mobile-link" @click="menuOpen = false">🗓 Calendário Vacinal</RouterLink>
...
.dot-1 { background: var(--coral-500); }
.dot-2 { background: var(--amber-400); width: 10px; height: 10px; }
.dot-3 { background: var(--blue-400); }
.logo-text { color: var(--text-main); }
.logo-text em { color: var(--coral-500); font-style: italic; }
...
.btn-nav {
  padding: 10px 22px; background: var(--coral-500); color: var(--white);
  border-radius: var(--radius-full); font-size: 0.875rem; font-weight: 600;
  transition: all 0.2s; white-space: nowrap;
  box-shadow: 0 4px 12px rgba(232, 93, 58, 0.25);
}
.btn-nav:hover { background: var(--coral-400); transform: translateY(-1px); }
...
.mobile-link:hover { background: var(--coral-50); color: var(--coral-500); }

@media (max-width: 640px) {
  .nav-links, .btn-nav { display: none; }
  .menu-toggle {
    display: block; margin-left: auto;
    width: 36px; height: 36px; background: var(--cream-dark);
