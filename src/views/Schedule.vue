<template>
  <div class="schedule-page">
    ...
    <div class="container page-body">
      ...
      <div class="filter-card">
        ...
        <div class="filter-right">
          ...
          <div class="input-group">
            <input v-model.number="filterValue" type="number" min="0" :max="unit === 'meses' ? 11 : 18"
              :placeholder="unit === 'meses' ? 'Ex: 6' : 'Ex: 4'" class="filter-input" />
            <span>{{ unit }}</span>
          ...
          <div class="quick-ages">
            <button v-for="q in quickAges" :key="q.label" :class="['quick-btn', { active: isQuickActive(q) }]"
              @click="applyQuick(q)">
              {{ q.label }}
            </button>
          </div>
      ...
      <!-- Timeline -->
      <div class="timeline">
        <div v-for="(item, i) in filteredSchedule" :key="i" class="timeline-item"
          ...
          <div class="timeline-content">
            ...
            <div class="vaccines-list">
              <RouterLink v-for="vid in item.vaccine_ids" :key="vid" :to="`/vacina/${getVaccine(vid)?.slug}`"
                class="vaccine-chip" :class="getVaccine(vid)?.sus ? 'chip-sus' : 'chip-private'">
...
const {
  filterValue, unit, showGestante,
  filterAgeMonths, filteredSchedule, quickAges,
  switchUnit, applyQuick, isQuickActive, clearFilter,
  getVaccine, isCurrentAge,
} = useCalendario()
