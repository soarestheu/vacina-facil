...
onMounted(() => {
    const queryAge = Number(route.query.idade);
    if (!queryAge) return;

    const isYears = queryAge >= MONTHS_PER_YEAR;
    unit.value = isYears ? "anos" : "meses";
    filterValue.value = isYears ? Math.round(queryAge / MONTHS_PER_YEAR) : queryAge;
});
</script>
        ...
        <div class="container page-body">
            <div class="filter-card">
                ...
                <div class="filter-right">
                    <div class="unit-toggle">
                        <button :class="['toggle-btn', { active: unit === 'meses' }]" @click="switchUnit('meses')">
                            Meses
                        </button>
                        <button :class="['toggle-btn', { active: unit === 'anos' }]" @click="switchUnit('anos')">
                            Anos
                        </button>
                    </div>
                    ...
                    <div class="input-group">
                        ...
                        />
                        <span>{{ unit }}</span>
                        <button v-if="filterValue !== null" class="clear-btn" @click="clearFilter">✕ Limpar</button>
                    </div>

                    <div class="quick-ages">
                        <button
                            v-for="quickAge in quickAges"
                            :key="quickAge.label"
                            :class="['quick-btn', { active: isQuickActive(quickAge) }]"
                            @click="applyQuick(quickAge)"
                        >
                            {{ quickAge.label }}
                        </button>
                    </div>
            ...
            <div class="timeline">
                <div
                    ...
                    :key="index"
                    class="timeline-item"
                    :class="{ highlight: isCurrentAge(scheduleItem), 'is-gestante': scheduleItem.gestante }"
                >
                    <div class="timeline-marker">
                        <div class="marker-dot" :class="{ 'dot-gestante': scheduleItem.gestante }"></div>
                        <div class="marker-line" v-if="index < filteredSchedule.length - 1"></div>
                    </div>

                    <div class="timeline-content">
                        <div class="timeline-header">
                            <h3>{{ scheduleItem.age_label }}</h3>
                            <span v-if="scheduleItem.gestante" class="gestante-badge">🤰 Gestantes</span>
                            <span v-else-if="isCurrentAge(scheduleItem)" class="now-badge">✨ Agora</span>
                        </div>
                        <p v-if="scheduleItem.notes" class="timeline-notes">{{ scheduleItem.notes }}</p>
                        <div class="vaccines-list">
                            <RouterLink
                                v-for="vaccineId in scheduleItem.vaccine_ids"
                                :key="vaccineId"
                                :to="`/vacina/${getVaccine(vaccineId)?.slug}`"
                                class="vaccine-chip"
                                :class="getVaccine(vaccineId)?.sus ? 'chip-sus' : 'chip-private'"
                            >
                                <span class="chip-name">{{ getVaccine(vaccineId)?.name }}</span>
                                <span class="chip-badge">{{ getVaccine(vaccineId)?.sus ? "SUS" : "Privada" }}</span>
                            </RouterLink>
                        </div>
                    </div>
                </div>
...
.gestante-toggle:hover {
    border-color: var(--coral-200);
}

.gestante-toggle-left {
    display: flex;
    align-items: center;
    gap: 14px;
}
