<script setup>
import { findVaccineBySlug, schedule } from "@/data/vaccines.js";
...
const vaccine = computed(() => findVaccineBySlug(route.params.slug));
...
const regularSchedule = computed(() => {
    ...
    return schedule.filter(
        (scheduleItem) => !scheduleItem.gestante && scheduleItem.vaccine_ids.includes(vaccine.value.id),
    );
});
...
const hasNoSchedule = computed(() => regularSchedule.value.length === 0 && gestanteSchedule.value.length === 0);
        ...
        <div class="container" v-if="vaccine">
            ...
            <div class="detail-hero">
                <div class="detail-left">
                    <div class="detail-badges">
                        ...
                        <span v-if="vaccine.gestante" class="avail-badge badge-gestante"
                            >🤰 Indicada para gestantes</span
                        >
                    </div>
                    <h1>{{ vaccine.name }}</h1>
                    <p class="manufacturer">
                        Fabricante: <strong>{{ vaccine.manufacturer }}</strong>
                    </p>
                    <div class="diseases-section">
                        <p class="diseases-label">Protege contra:</p>
                        <div class="diseases-list">
                            <span v-for="disease in vaccine.diseases" :key="disease" class="disease-pill">{{
                                disease
                            }}</span>
                        </div>
                    </div>
                ...
                <div class="detail-right">
                    <div class="schedule-card">
                        ...
                        <div v-if="regularSchedule.length > 0">
                            <div
                                v-for="scheduleItem in regularSchedule"
                                :key="scheduleItem.age_label"
                                class="schedule-row"
                            >
                                <span class="schedule-age">{{ scheduleItem.age_label }}</span>
                                <span class="schedule-note">{{ scheduleItem.notes }}</span>
                            </div>
                        </div>
                        <div v-if="gestanteSchedule.length > 0" class="gestante-block">
                            <div class="gestante-block-title">🤰 Para gestantes</div>
                            <div
                                v-for="scheduleItem in gestanteSchedule"
                                :key="scheduleItem.age_label"
                                class="schedule-row"
                            >
                                <span class="schedule-age">{{ scheduleItem.age_label }}</span>
                        ...
                        <p v-if="hasNoSchedule" class="no-schedule">
            ...
            <div class="prices-section">
                ...
                <div v-if="loadingPrices" class="price-placeholder">
                    <div class="placeholder-card" v-for="skeletonIndex in 3" :key="skeletonIndex">
                ...
                <div v-else class="prices-list">
                    <div
                        v-for="(clinicPrice, index) in clinicPrices"
                        :key="clinicPrice.clinics.id"
                        class="price-card"
                        :class="{ 'price-best': index === 0 }"
                    >
                        <div class="price-card-left">
                            <span v-if="index === 0" class="best-badge">🏆 Menor preço</span>
                            <h4>{{ clinicPrice.clinics.name }}</h4>
                            <p class="clinic-address">
                                📍 {{ clinicPrice.clinics.address }} · {{ clinicPrice.clinics.city }}/{{
                                    clinicPrice.clinics.state
                                }}
                            </p>
                            <a
                                v-if="clinicPrice.clinics.phone"
                                :href="`tel:${clinicPrice.clinics.phone}`"
                                class="clinic-phone"
                            >
                                📞 {{ clinicPrice.clinics.phone }}
                            </a>
                        </div>
                        <div class="price-card-right">
                            <span class="price-value">{{ formatPrice(clinicPrice.price) }}</span>
                            <span class="price-updated">Atualizado em {{ formatDate(clinicPrice.updated_at) }}</span>
                        </div>
                    </div>
                </div>
